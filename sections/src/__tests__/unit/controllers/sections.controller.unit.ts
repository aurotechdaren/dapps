...
describe('SectionsController (unit)', () => {
  ...
  describe('deleteById()', () => {
    it('successfully deletes existing items', async () => {
      deleteById.resolves();
      await controller.deleteById(aSectionsWithId.id as string);
  ...
  describe('count()', () => {
    it('returns the number of existing sectionsLists', async () => {
      ...
      expect(await controller.count()).to.eql(aListOfSectionss.length);
  ...
  describe('updateAll()', () => {
    it('returns a number of sectionss updated', async () => {
      ...
      expect(await controller.updateAll(aChangedSections, where)).to.eql(1);
