...
describe('FormsController (unit)', () => {
  ...
  describe('createForms', () => {
    it('creates a Forms', async () => {
      ...
      const result = await controller.create(aForms);
  ...
  describe('findFormsById', () => {
    it('returns a forms if it exists', async () => {
      ...
      expect(await controller.findById(aFormsWithId.id as string)).to.eql(
        aFormsWithId,
      );
  ...
  describe('findFormss', () => {
    it('returns multiple formss if they exist', async () => {
      ...
      expect(await controller.find({})).to.eql(aListOfFormss);
  ...
  describe('replaceForms', () => {
    it('successfully replaces existing items', async () => {
      ...
      await controller.replaceById(aFormsWithId.id as string, aChangedForms);
  ...
  describe('updateForms', () => {
    it('successfully updates existing items', async () => {
      updateById.resolves();
      await controller.updateById(aFormsWithId.id as string, aChangedForms);
  ...
  describe('deleteById()', () => {
    it('successfully deletes existing items', async () => {
      deleteById.resolves();
      await controller.deleteById(aFormsWithId.id as string);
  ...
  describe('count()', () => {
    it('returns the number of existing formsLists', async () => {
      ...
      expect(await controller.count()).to.eql(aListOfFormss.length);
  ...
  describe('updateAll()', () => {
    it('returns a number of formss updated', async () => {
      ...
      expect(await controller.updateAll(aChangedForms, where)).to.eql(1);
  ...
  function resetRepositories() {
    ...
    controller = new FormsController(formsRepo);
  }
});
