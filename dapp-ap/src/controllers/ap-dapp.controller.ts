import {param, patch, put, requestBody, post, get} from '@loopback/rest';
...
export class ApDappController {
  ...
  async create(@requestBody({required: false}) ap: Ap): Promise<Ap> {
    ...
    try {
      ...
      let sowJson = JSON.stringify({});
      let sow = await this.sowRepository.create({body: sowJson});

      let evaluationCriteriaJson = JSON.stringify({});
      let evaluationCriteria = await this.evaluationCriteriaRepository.create({
        body: evaluationCriteriaJson,
      });
  ...
  async updateSow(
    @param.path.string('ap_no') ap_no: string,
    @requestBody() sow: Sow,
  ): Promise<void> {
  ...
  async assetFromBlockChain(
    @param.path.string('asset_no') asset_no: string,
  ): Promise<void> {
  ...
  async acceptAp(
    ...
    try {
      ...
      //let createAPResponse =
      await this.blockchainRepository.createAP({
        data: JSON.parse(JSON.stringify(assetAP)),
      });
      ...
      //let createAPHashResponse =
      await this.blockchainRepository.createAPHash({
        data: JSON.parse(JSON.stringify(hashAP)),
      });
  ...
  async initiateAp(
    ...
    try {
      ...
      let initiateAPResponse = await this.blockchainRepository.initiateAP({
        data: JSON.parse(JSON.stringify(initiateAP)),
      });
